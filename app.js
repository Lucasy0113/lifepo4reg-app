...
function setupAuthListener() {
  ...
  window.db.supabase.auth.onAuthStateChange((event, session) => {
    ...
    else if (event === 'SIGNED_OUT') { currentUser = null; batteriesCache = []; readingsCache = []; localStorage.removeItem('lifepo4_user'); hideLoading(); document.getElementById('user-menu-btn').style.display = 'none'; if($addBtn) $addBtn.style.display = 'none'; renderAll(); showLoginModal(); }
  });
}
...
async function loadData() {
  ...
  try {
    ...
    readingsCache = [];
    if (batteriesCache.length > 0) { const allReadings = await Promise.all(batteriesCache.map(b => window.db.fetchReadings(b.id))); readingsCache = allReadings.flat(); }
    updateFilterDropdown(); renderAll();
  } catch (e) { console.error('❌ Error cargando:', e); renderAll(); }
}
function updateFilterDropdown() { $batteryFilter.innerHTML = '<option value="all">Todas las baterías</option>'; batteriesCache.forEach(b => { const opt = document.createElement('option'); opt.value = b.id; opt.textContent = b.name + (b.model ? ` (${b.model})` : ''); $batteryFilter.appendChild(opt); }); if (selectedBatteryId && batteriesCache.some(b=>b.id===selectedBatteryId)) $batteryFilter.value = selectedBatteryId; }

// ✅ TABS & EVENTS CORREGIDOS
function setupEvents() {
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (tab === 'readings' && !selectedBatteryId) {
        if (batteriesCache.length > 0) { selectedBatteryId = batteriesCache[0].id; $batteryFilter.value = selectedBatteryId; }
        else { alert("Crea o selecciona una batería primero."); currentTab = 'dashboard'; document.querySelector('.tab-btn[data-tab="dashboard"]').click(); return; }
      }
      renderAll();
    });
  });
  ...
  $list?.addEventListener('click', (e) => {
    ...
    e.preventDefault(); e.stopPropagation(); // ✅ Evita doble disparo
    const card = target.closest('.battery-card, .reading-card');
    const id = card?.dataset.id;
    if (target.classList.contains('btn-readings')) { selectedBatteryId = id; currentTab = 'readings'; currentPage=1; document.querySelector('.tab-btn[data-tab="readings"]').click(); return; }
...
function renderReadingsList() {
  ...
  let html = `<div class="reading-header">🔋 ${bat.name} ${bat.model?`(${bat.model})`:''}</div><button onclick="currentTab='dashboard'; selectedBatteryId=null; document.querySelector('.tab-btn[data-tab=dashboard]').click(); renderAll();" style="margin-bottom:1rem; padding:0.4rem 0.8rem; background:var(--card); border:1px solid var(--border); border-radius:6px; cursor:pointer;">← Volver</button>`;
...
function openReadingModal(id=null) { if ($modalActions) $modalActions.style.display = 'flex'; editingId = id; const bat = batteriesCache.find(b=>b.id===selectedBatteryId); const read = id ? readingsCache.find(r=>r.id===id) : null; document.getElementById('modal-title').textContent = id ? 'Editar Lectura' : 'Nueva Lectura'; $fields.innerHTML = ''; const now = getCubaNowISO(); appendField({id:'recorded_at', label:'Fecha/Hora Lectura', type:'datetime-local', val: read?.recorded_at?.slice(0,16) || now}); let html = `<div id="cells-container"><label style="display:block; margin-bottom:0.3rem; font-weight:500;">Voltaje por Celda (2.500 - 3.650 V)</label>`; for(let i=0; i<bat.cell_count; i++) { const val = read?.voltages ? (read.voltages[i]||'') : ''; html += `<input type="number" step="0.001" id="cell_${i}" placeholder="Cel ${i+1} (V)" value="${val}" required style="margin-bottom:0.3rem;">`; } html += `</div><div style="margin-top:0.5rem;"><label style="display:block; margin-bottom:0.2rem; font-weight:500;">Cargador V</label><input type="text" id="charger_v" value="${read?.charger_v || 'MPPT'}" style="margin-bottom:0.5rem;"><label style="display:block; margin-bottom:0.2rem; font-weight:500;">Cargador A</label><input type="text" id="charger_a" value="${read?.charger_a || 'MPPT'}"></div>`; $fields.insertAdjacentHTML('beforeend', html); $modal?.showModal(); }

// ✅ SAVE RECORD CORREGIDO (Sin lecturas fantasmas al editar, inserta correctamente)
...
// ✅ DELETE READING CORREGIDO (Una sola confirmación, refresco forzado)
async function deleteReading(id) {
  if(!confirm('¿Eliminar esta lectura permanentemente?')) return;
  showLoading();
  try {
    await window.db.deleteRecord('cell_readings', id);
    readingsCache = readingsCache.filter(r => r.id !== id); // Optimista
    await loadDataReadings(); // Refresco real desde BD
    renderAll();
  } catch(err) { alert('Error: '+err.message); } finally { hideLoading(); }
}
...
function showLoginModal() { 
  if ($modalActions) $modalActions.style.display = 'none'; document.getElementById('modal-title').textContent = 'Iniciar Sesión';
  $fields.innerHTML = `<div style="margin-bottom:1rem"><label>Email</label><input type="email" id="auth-email" required></div><div style="margin-bottom:1rem"><label>Contraseña</label><input type="password" id="auth-pass" minlength="6" required></div><div style="display:flex;gap:0.5rem"><button type="button" id="auth-login" style="flex:1;padding:0.6rem;background:#dbeafe;color:#1d4ed8;border:none;border-radius:6px;">Entrar</button><button type="button" id="auth-signup" style="flex:1;padding:0.6rem;background:#fee2e2;color:#b91c1c;border:none;border-radius:6px;">Registrarse</button></div><p id="auth-error" style="color:#ef4444;font-size:0.85rem;margin-top:0.5rem;display:none;"></p>`;
  $modal?.showModal(); document.getElementById('auth-login').onclick = async () => handleAuth('login'); document.getElementById('auth-signup').onclick = async () => handleAuth('signup');
}
async function handleAuth(type) {
  const e = document.getElementById('auth-email').value, p = document.getElementById('auth-pass').value; const err = document.getElementById('auth-error');
