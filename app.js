...
function setupAuthListener() {
    ...
    window.db.supabase.auth.onAuthStateChange((event, session) => {
        if (event === 'SIGNED_IN' && session?.user) {
            ...
            currentUser = null; batteriesCache = []; readingsMap = {};
...
function renderReadingsList() {
    ...
    if (!bat) { $list.innerHTML = '<div class="empty-state">Batería no encontrada.</div>'; return; }
    ...
    let html = `<div style="text-align:center; margin-bottom:1.5rem; padding:1rem; background:var(--card); border-radius:8px; box-shadow:var(--shadow);">
        <div style="font-size:1.2rem; font-weight:700; color:var(--primary); margin-bottom:0.3rem;">🔋 ${bat.name} ${bat.model?`(${bat.model})`:''}</div>
        <div style="color:var(--text-sec); font-size:0.9rem;">${bat.total_voltage}V / ${bat.amperage}Ah</div>
    </div>
    <button onclick="goBackToPanel()" style="margin-bottom:1rem; padding:0.4rem 0.8rem; background:var(--card); border:1px solid var(--border); border-radius:6px; cursor:pointer;">← Volver al Panel</button>`;

    if(!pageData.length) {
        html += '<div class="empty-state">Sin lecturas registradas.</div>';
    } else {
        pageData.forEach(r => {
